...
for item in *; do
	...
	results_length=${#results[@]}										 # store the number of matches from mdfind
		...
		echo -e "The folder '$item' s been found elsewhere by spotlight ...: "				# output mdfind matches

		printf "%-20s %-30s %-30s" "$count1. local folder > " "size:$local_size/count:$local_file_count_trimmed" "" 
		
	 	printf ".. here are the other folders > \n"							# get ready to display duplicates found by mdfind
		
		# now, display the duplicated discovered by mdfind, after fixing the formatting etc ...
...
## rejig 1			# this loop checks if the result matches the original THEN if not, compares the discovered duplicate
## rejig 1	 		if [ "$count1" = "1" ]; then open_finder; count1=$(( $count1 + 1 )); fi
## rejig 1			if [ "$real_path" = "$result" ] && [ "$results_length" -eq "1" ]; then          # make sure mdfind does not return the original item
## rejig 1				printf "%-20s %-30s %-30s\n" "No duplicates found in mdfind...!" "" ""
## rejig 1			elif [ "$local_size" -eq "$remote_size" ] ; then
## rejig 1			  comparison="same"
## rejig 1			  #printf "%-20s %-30s %-30s\n" "${count1}.   $comparison" "size:${remote_size}/count:$remote_file_count_trimmed" "rm -r \"$result\""
## rejig 1			  printf "%-20s %-30s %-30s\n" "${count1}.   $comparison" "size:${remote_size}/count:$remote_file_count_trimmed" "" #"rm -r \"$result\""
## rejig 1				open_finder	
## rejig 1				count1=$(( $count1 + 1 ))
## rejig 1			else
## rejig 1				comparison="diff"
## rejig 1		  	#printf "%-20s %-30s %-30s\n" "${count1}.   $comparison" "size:${remote_size}/count:$remote_file_count_trimmed" "rm -r \"$result\""
## rejig 1			printf "%-20s %-30s %-30s\n" "${count1}.   $comparison" "size:${remote_size}/count:$remote_file_count_trimmed" "" #"rm -r \"$result\""
## rejig 1				open_finder	
## rejig 1				count1=$(( $count1 + 1 ))
