...
#						|| myping1="xxx"
	myping1=$(ping -t 2 -c 1 $IP1 | grep -E '(icmp_seq)'| awk -F"=" '{print $4}') > /dev/null 2>&1   || myping1="xxx"


#	ping -c1 $IP2 > /dev/null 2>&1 		&& myping2=`ping -t 2 -c 1 $IP2 | egrep '(icmp_seq)'| awk -F"=" '{print $4}'`  \
#						|| myping2="xxx"
	myping2=$(ping -t 2 -c 1 $IP2 | grep -E '(icmp_seq)'| awk -F"=" '{print $4}') > /dev/null 2>&1   || myping2="xxx"

#	ping -c1 $IP3 > /dev/null 2>&1 		&& myping3=`ping -t 2 -c 1 $IP3 | egrep '(icmp_seq)'| awk -F"=" '{print $4}'`  \
#						|| myping3="xxx"
	myping3=$(ping -t 2 -c 1 $IP3 | grep -E '(icmp_seq)'| awk -F"=" '{print $4}') > /dev/null 2>&1   || myping3="xxx"
...
#						|| { myping4="xxx" ; bar1="error";  }
	{ myping4=$(ping -t 2 -c 1 $IP4 | grep -E '(icmp_seq)'| awk -F"=" '{print $4}');  \
						   myping4a=$(echo "$myping4" | awk '{print $1}'); \
						   [ -f "$BASE2/$BAR" ] && bar1=$($BASE2/$BAR "$myping4a" $BARLENGTH); }  \
						|| { myping4="xxx" ; bar1="error";  }
